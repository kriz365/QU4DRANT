...
var tailwindJsonOutfile = path2.resolve(
  projectRoot,
  "./src/tailwind.config.superdev.json"
);
var tailwindIntermediateFile = path2.resolve(
  projectRoot,
  "./.superdev.tailwind.config.js"
);
var isSandbox = process.env.SUPERDEV_SANDBOX === "true";
  ...
  return {
        ...
        walk(ast, {
          enter(node) {
            ...
            if (node.type === "JSXOpeningElement") {
              ...
              const attributes = jsxNode.attributes.reduce(
                (acc, attr) => {
                  ...
                  if (attr.type === "JSXAttribute") {
                    if (((_a2 = attr.value) == null ? void 0 : _a2.type) === "StringLiteral") {
                      acc[attr.name.name] = attr.value.value;
                    } else if (((_b2 = attr.value) == null ? void 0 : _b2.type) === "JSXExpressionContainer" && attr.value.expression.type === "StringLiteral") {
              ...
              const line = (_c = (_b = (_a = jsxNode.loc) == null ? void 0 : _a.start) == null ? void 0 : _b.line) != null ? _c : 0;
              const col = (_f = (_e = (_d = jsxNode.loc) == null ? void 0 : _d.start) == null ? void 0 : _e.column) != null ? _f : 0;
              const dataComponentId = `${relativePath}:${line}:${col}`;
              const fileName = path2.basename(id);
              const shouldTag = shouldTagElement(elementName);
              if (shouldTag) {
                const oldIds = ` data-component-path="${relativePath}" data-component-line="${line}" data-component-file="${fileName}" data-component-name="${elementName}" data-component-content="${encodeURIComponent(
                  JSON.stringify(content)
                )}"`;
                magicString.appendLeft(
                  (_g = jsxNode.name.end) != null ? _g : 0,
                  ` data-superdev-id="${dataComponentId}" data-superdev-name="${elementName}" ${oldIds}`
                );
    ...
    async buildStart() {
      ...
      try {
        ...
        console.error("Error generating tailwind.config.json:", error);
      }
    },
